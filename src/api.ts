...
export default class TestRailAPI {
    ...
    async updateTestRun(runId: string, case_ids: unknown[]) {
        await axios
            .get(`${this.#baseUrl}/get_tests/${runId}`, this.#config)
            .then((res) => {
                if (res.data.tests.length > 0) {
                    const addCaseIds = res.data.tests.map(
                        (tests: { case_id: number }) => tests.case_id
                    );
    ...
    async pushResults(testId: string, results: TestResults) {
        try {
            const resp = axios.post(
                `${this.#baseUrl}/add_result_for_case/${
                    this.#projectId
                }/${testId}`,
                results,
                this.#config
    ...
    async getLastTestRun(
        ...
        try {
            const resp = await axios.get(
                `${this.#baseUrl}/get_runs/${
                    this.#projectId
                }&is_completed=0&created_after=${unixTimeStamp}&suite_id=${suiteId}`,
                this.#config
            );
